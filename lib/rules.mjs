 ...
 *     `key: "x"` are both valid; **742** vanilla entries are of the second shape. Requiring
 *     `\d*\s+` flagged every one of them, and the wiki agrees the counter is deprecated for
 *     modders (`Localization`: "The number after the : is optional and it does nothing for
 *     modders… completely deprecated").
 ...
 * @returns `{ available, reason?, databasePath, mods, playset }`.
 */
export async function readLauncherState(launcherDir) {
  const databasePath = path.join(launcherDir, 'launcher-v2.sqlite')
 ...
 *
 * Three disagreements are worth reporting, and each has a distinct cause:
 *   * the launcher shows a mod whose `.mod` file is gone → a dead entry in the picker;
 *   * a `.mod` file on disk that the launcher has never registered → the launcher will not
 *     recognise it (the wiki: without the sibling `.mod` "the launcher will not recognise the mod");
 ...
 * "be fired from script in some way", and the engine writes an `event_log.csv` counting, per event,
 * **how many times it was checked and how many times each option was picked** (Patch 1.5: *"the # of
 * times each event has been checked, and the # of times each option has gotten picked"*). A row with
 * `checked = 0` is an event wired to nothing, and no static parse can produce that.
 *
 ...
 *      game looks like while it is still loading: the engine creates all sixteen files at startup and
 *      flushes as it goes. **An empty `error.log` is not "no errors"** — it is "nothing flushed yet",
 *      and the two are indistinguishable from the bytes alone.
 *   3. **Populated** — and here is the measured part. **A non-empty log is NOT evidence of a problem.**
 *      Observed on a clean, unmodded launch: `setup.log` reported
 *      `[W][provincetemplate.cpp:158]: Province 10186 has no pixels!` — vanilla's own warnings, with no
 *      mod enabled at all. The wiki says the same in words: *"the log will report errors even in an
 *      unmodded game. Launch the game without any mods and let it run for a while to learn which
 *      errors are common and not caused by you."* So this reader **counts and reports**; it raises a
 *      finding only for the event-reachability signal, which concerns the author's own content.
 *
 * @param logsDir - the launcher user directory's `logs` subdirectory.
 * @returns `{ available, flushed, files, eventReport, findings }`.
 */
export async function readRuntimeEvidence(logsDir) {
  ...
  /*
   * DEDUPLICATE ACROSS SINKS. Measured on a real launch: the SAME two E-level messages appeared in
   * `debug.log`, `error.log` AND `game.log` — the engine fans one message out to several sinks. A
   * per-file count therefore reported "6 E-level" for two distinct errors, inflating by 3x the only
   * number a reader would act on. Messages are keyed by their text with the `[HH:MM:SS]` prefix
   * stripped, and the contributing sinks are kept so the report can say where each was found.
   */
  ...
  for (const file of files) {
    ...
    for (const line of splitLines(file.text)) {
      ...
      const key = parsed.message.trim()
      if (key === '') continue
      if (!distinct.has(key)) distinct.set(key, { message: key, source: parsed.source, files: [] })
      const entry = distinct.get(key)
      if (!entry.files.includes(file.name)) entry.files.push(file.name)
    }
  }
  const distinctErrors = [...distinct.values()]
  if (eventReport !== null) {
  ...
  return { available: true, flushed, files, eventReport, distinctErrors, findings }
}
